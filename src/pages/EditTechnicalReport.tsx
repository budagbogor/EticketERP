...
export default function EditTechnicalReport() {
  ...
  // Fetch existing technical report
  ...
  const handleSubmit = async () => {
    if (!existingReport) {
      toast({
        title: "Error",
        description: "Laporan teknik tidak ditemukan",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      await supabase.from("complaint_history").insert({
        ...
        description: `Laporan teknik diperbarui oleh ${formData.picName}`,
        performed_by: user?.id || "",
      });

      // Invalidate queries
      queryClient.invalidateQueries({ queryKey: ["technical_report", ticketId] });
      queryClient.invalidateQueries({ queryKey: ["closed-complaints-with-history"] });

      toast({
        title: "Berhasil",
        description: "Laporan teknik berhasil diperbarui",
      });
  ...
  if (ticketLoading || reportLoading) {
    return (
  ...
  if (!existingReport) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px]">
        <p className="text-muted-foreground">Laporan teknik tidak ditemukan</p>
        <Button variant="link" onClick={() => navigate(`/tickets/${ticketId}`)}>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold">Edit Laporan Teknik</h1>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">Form Edit Laporan Teknik</CardTitle>
        </CardHeader>
